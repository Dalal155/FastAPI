...
def check_for_duplicates(initiative: Initiative):
    ...
    input_text = advanced_preprocess(initiative.mobadrahName + " " + initiative.briefDescription)
...
# Endpoint to "register" initiative (no DB insertion)
...
async def register_initiative(initiative: Initiative):
    try:
        ...
        if duplicate_indices:
            ...
            raise HTTPException(
                status_code=400,
                detail="The initiative can't be registered because there is another initiative with the same idea."
            )
